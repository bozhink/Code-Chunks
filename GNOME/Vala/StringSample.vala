...
void main ()
{
     ...
     */
      ...
      * What its help you are able to build strings ad libitum by prepending,
      ...
      */
       ...
       * method 'printf' on it. Format strings follow the usual rules, known
       * from C and similar programming languages.
       */
        ...
        */
         ...
         * Java's behaviour which in this case would check for referental equality.
         */
         ...
         switch (pl)
         {
             case "java": assert_not_reached ();
             case "vala":
                  println ("Switch statement works fine with strings.");
                  break;
             case "ruby": assert_not_reached ();
         }
         
         /*
          * Vala offers a feature called verbatim strings. These are strings in
          * which escape sequence (such as \n) won't be interpreted, line breaks
          * will be presented and quotation marks don't have to be masked. They are
          * enclosed with triple double quotation marks. Possible identations
          * after a line break are part of the string as well.
          */
          string verbatim = """This is so-called "verbatim string".
 ...
 * String Length
 * 
 * Before Vala 0.11 .length returned the number of Unicode code points of a string and .size() returned the length in bytes: 
 * 
 * string dessert = "crème brûlée";
 * assert (dessert.length == 12);
 * assert (dessert.size () == 15);
 * 
 * Starting with Vala 0.11 .length returns the length in bytes and index access is byte-based as well: 
 * 
 * string dessert = "crème brûlée";
 ...
 * assert (a_byte == 0xA8);
 * 
 * Note that strings in Vala are still UTF-8, nothing changed in that regard. The difference is that 
 * indices/offsets/lengths are measured in code units (bytes as we are talking about UTF-8) instead of code points. 
 * 
 * This is in line with string APIs in many libraries (Java, .NET, Qt, Go) and makes it possible to improve
 * performance in string handling without resorting to pointers all the time while still properly supporting Unicode strings. 
 * 
