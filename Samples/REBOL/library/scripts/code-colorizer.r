REBOL [
    ...
    Date:    12-Mar-2008/15:43:13+1:00
...
unless value? 'seo-name [
	seo-name: func [
	    ...
	    trans-chars:  charset "ÏÈÎöúËÊ¯‡ûü˝·‰ÌÈÛˆÔùÚ˙˘¸"
...
;** These are groups with words used in Rebol
	...
	rl_math: [
		...
		"log-10" | "log-2" | "log-e" | "max" | "maximum" | "maximum-of" | "min" | "minimum" |
	...
	rl_datatype: [
		...
		"string?" | "struct?" | "tag?" | "time?" | "to" | "to-binary" | "to-bitset" | "to-block" |
		...
		"to-word" | "tuple?" | "type?" | "unset?" | "url?" | "word?"
	]
	rl_view: [
		"alert" | "as-pair" | "brightness?" | "caret-to-offset" | "center-face" | "choose" | "clear-fields" |
		"do-events" | "dump-face" | "flash" | "focus" | "hide" | "hide-popup" | "in-window?" | "inform" |
		"layout" | "link?" | "load-image" | "make-face" | "offset-to-caret" | "request" | "request-color" |
		"request-date" | "request-download" | "request-file" | "request-list" | "request-pass" | "request-text" |
		"show" | "show-popup" | "size-text" | "span?" | "stylize" | "unfocus" | "unview" | "view" | "viewed?" | "within?"
	]
...
;## colorize
	set 'colorize func[source /save outfile /title ttl /local source-file text x][
						...
						case [
							...
							true [
								parse x [
									...
									| rl_comparison end (emit/class x 'kw2)
									| rl_context    end (emit/class x 'kw3)
									| rl_control    end (emit/class x 'kw4)
									| rl_help       end (emit/class x 'kw5)
									| rl_logic      end (emit/class x 'kw6)
									| rl_math       end (emit/class x 'kw7)
									| rl_io         end (emit/class x 'kw8)
									| rl_series     end (emit/class x 'kw9)
									| rl_dataset    end (emit/class x 'kw10)
									| rl_specialstring end (emit/class x 'kw11)
									| rl_system     end (emit/class x 'kw12)
									| rl_datatype   end (emit/class x 'kw13)
									| rl_view       end (emit/class x 'kw14)
									| (emit x)
								]
	    ...
	    if save [
	    	...
	    	write/binary outfile rejoin [
	    		...
	    		either none? source-file [""][
	    			rejoin [
	    				{Source: <b>} last split-path source-file {</b> modified: <b>} modified? source-file {</b>}
	    			]
	    		]
