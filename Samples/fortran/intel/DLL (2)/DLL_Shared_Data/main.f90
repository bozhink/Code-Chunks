...
! DLL_Shared_Data example program for Intel Visual Fortran
!
...
if (ret /= ERROR_ALREADY_EXISTS) then
  ...
  mainloop: do
    ...
    write (*,'(A,G13.6)') 'Child process returned SQRT value of ', shared_variable
  ...
  childloop: do
    cwaitloop: do i=100, 0, -1
      ...
      select case (ret)
        ...
        case (WAIT_TIMEOUT)
          ...
          write (*,'(A)') 'Timeout - iving up on parent'
          exit childloop
        case (WAIT_FAILED)
          write (*, '(A,I0)') 'Wait failed, status = ', GetLastError()
          exit childloop
    ...
    write (*,'(A,G10.5)') 'Received value ', shared_variable
