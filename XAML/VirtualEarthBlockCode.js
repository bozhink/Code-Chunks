...
VirtualEarthClass.prototype.initialize = function(callback) {
// Shows and initializes the map.

    if (environment.designTime) {
        throw "Virtual Earth doesn't support initialize at design time.";
    }
    ...
    {
        window.setTimeout(delegate(this, function() { this.initialize(callback); }), 1000/30);
    ...
    mapResizer(); // initial resize
    $addHandler(window, "resize", mapResizer); // resize the map when the window is resized
...
VirtualEarthClass.prototype.addPushpin = function(latitude, longitude, url, title, description, centerMapOnPushpin) {
