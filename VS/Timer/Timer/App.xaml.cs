...
namespace Timer
{
    public partial class App : Application
    {
        ...
        const string SAVEFILENAME = "x.txt";
                ...
                if (savegameStorage.FileExists(SAVEFILENAME))
                {
                    using (IsolatedStorageFileStream fs = savegameStorage.OpenFile(SAVEFILENAME, System.IO.FileMode.Open))
                    {
                        if (fs != null)
                        {
                            ...
                            count = fs.Read(saveBytes, 0, saveBytes.Length);
                            textContent = string.Empty;
                            if (count > 0)
                            {
                                textContent = System.Text.Encoding.UTF8.GetString(saveBytes, 0, saveBytes.Length);
                            }
        ...
        private void Application_Closing(object sender, ClosingEventArgs e)
        {
            ...
            using (fs = savegameStorage.CreateFile(SAVEFILENAME))
            {
                if (fs != null)
                {
                    ...
                    {
                        sb.Append(item.LineOne);
                        sb.Append("\t");
                        sb.Append(item.LineTwo);
                        sb.Append("\t");
                        sb.Append(item.LineThree);
                        sb.Append("\n");
                    }
