 ...
 * 
 * This program uses random-sequental update rule,
 * implemented by OpenMP parallelization.
 */
...
int main (int argc, char**argv) {
    int i, id, j, n, k, iter, curr, next, sum;
    double p, prand, riter;
    if (argc < 2) {
...
void update (int i, int j, double p) {
    if (chain_old[j]) p = 0.0;
    if ((chain[i]!=0) && (chain[j]==0) && ((1.0*rand()) / RAND_MAX < p)) {
        /* Transition */
        chain[j] = 1;
        chain[i] = 0;
        chain_old[i] = 1;
