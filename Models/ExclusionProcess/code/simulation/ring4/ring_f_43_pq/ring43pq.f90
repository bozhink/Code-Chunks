program ring43pq
    ...
    integer, parameter :: N = 3 ! Number of particles
    ...
    do j = L, 1, -1
       ...
       do kk = 1, K
          ...
          do iter = 1, NITER
             ...
             next = mod(L  +L+j, L) + 1
             call updateq(L, curr, next, p, q, prand, chain, chain_old, icurrent, moved0)
             do i = L-1, 2, -1
                curr = mod(i-1+L+j, L) + 1
                next = mod(i  +L+j, L) + 1
                call updateq(L, curr, next, p, q, prand, chain, chain_old, icurrent, moved)
             enddo
             if (.not.moved0) then
                curr = mod(L+L+j, L) + 1
                next = mod(1+L+j, L) + 1
   ...
   10     format(14('-'),'+',4(7('-'),'+'))
          print "('Out configurations ',4(2x,i5))", prob
       enddo
    enddo
