...
-------------------------------------------------------------------------------- 
  ...
  Put_Line("");
  for i in 1 .. N loop Put(Float'Image(spec(i)) & " "); end loop;
  Put_Line("");
  ...
  gauss_jordan( pm, 6, lambda, ierr);
  for i in 1 .. N loop
    vec(i):=pm(N,i);
  end loop;
  
  Put_Line("Error flag = " & Integer'Image(ierr));
  
  Put_Line("");
  Put_Line("Eigenvector");
  Put_Line("");
  for i in 1 .. N loop Put(Float'Image(vec(i)) & " "); end loop;
  Put_Line("");
  -----------------------------------
  for i in States loop
    for j in States loop
  ...
  for i in 1..N loop
    x(i):=0.0;
    for j in 1..N loop
      x(i):=x(i)+pm(j,i)*vec(j);
    end loop;
  end loop;
  Put_Line("");
  Put_Line("Eigenvector");
  Put_Line("");
  for i in 1 .. N loop Put(Float'Image(x(i)) & " "); end loop;
  Put_Line("");
  for i in 1 .. N loop Put(Float'Image(lambda*vec(i)) & " "); end loop;
  Put_Line("");
