...
c
      parameter(w0 =   3956.0d0 / 14175.0d0)
      parameter(w1 =  23552.0d0 / 14175.0d0)
      parameter(w2 =  -3712.0d0 / 14175.0d0)
      parameter(w3 =  41984.0d0 / 14175.0d0)
      parameter(w4 = -18160.0d0 / 14175.0d0)
c
      parameter(levmin = 1)
      levmax = 30
      levout = 6
...
c
   30 x(1) = (x0 + x(2)) * 0.5d0
      f(1) = fun(x(1))
      do 35 j = 3, 15, 2
         x(j) = (x(j-1) + x(j+1)) * 0.5d0
         f(j) = fun(x(j))
   35 continue
      nofun = nofun + 8
      step  = (x(16) - x0) * 0.0625d0
...
c
   ...
   82 temp = dabs(result) + errest
      ...
      subroutine quanc8d2(fun,y,a,b,abserr,relerr,
     *                    result,errest,nofun,flag)
      implicit none
c
      double precision fun,y,a,b,abserr,relerr,result,errest,flag
      integer nofun
c
c   estimate the integral of fun(x) from a to b
c   to a user provided tolerance.
