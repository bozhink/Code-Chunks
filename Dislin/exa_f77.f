      PROGRAM EXAMPL
      CHARACTER*4 CDEV,CDSP,C*1

      CALL GETDSP(CDSP)

      IF(CDSP.EQ.'XWIN') THEN
        CALL WIDGET(CDEV,IOPT)
      ELSE
10      CALL OPTION(CDEV,IOPT)
...
20    END
	...
	CALL EXA_14
      ...
      SUBROUTINE WIDGET(CDEV,IOPT)
      COMMON /CWIDGET/ CL1
      CHARACTER*(*) CDEV,CL1*650,CL2*600,CHELP*200
      INTEGER IRAY(2)
     ...
     *    'Line Styles|Legend|Shading Patterns (AREAF)|Vectors|'//
     *    'Shading Patterns (PIEGRF)|Surface Plot|Surface Plot|'//
     *    'Parametric Function Plot|'//
     *    'Map Plot|World Coastlines|Elliptical Proj.|'//
     *    'Azimuthal Proj.|'//
     *    'Conical Proj.|Bar Graphs|Pie Charts|3-D Bar/Pie Graphs|'//
     *    'Contours|Shaded Contours|'//
     *    'Instruction Alphabet|3-D Colour Plot|Shaded World|'//
     *    'Colour Tables'

      CDEV='XWIN'

      CALL SWGTIT('DISLIN Examples')
      CALL SWGHLP(CHELP)
      CALL SWGWTH(25)
      ...
      CALL WGINI('HORI',IP)
      CALL WGBAS(IP, 'VERT', IP_L)
      CALL WGBAS(IP, 'VERT', IP_R)

      CALL WGLAB(IP_L,'Example:',ID)
      CALL WGLIS(IP_L,CL2,IEXA,IRAY(1))
      CALL WGQUIT (IP_L,ID)

      CALL WGLAB(IP_R,'Device:',ID)
      CALL WGLIS(IP_R,CL1,IDEV,IRAY(2))

      CALL WGPBUT(IP_R, 'Plot', ID_PBUT)
      CALL SWGCB (ID_PBUT, MYSUB, IRAY)
      CALL WGFIN
      END

      SUBROUTINE MYSUB(ID,IRAY)
      COMMON /CWIDGET/ CL1
      CHARACTER*8 CDEV,CL1*620,CSTR*80
      INTEGER IRAY(2)

      CALL GWGLIS(IRAY(1),IOPT)
      CALL GWGLIS(IRAY(2),IDEV)
      CALL ITMSTR(CL1,IDEV,CSTR)
      CDEV=CSTR(1:4)

      CALL EXA(CDEV,IOPT)
      END

      SUBROUTINE OPTION(CDEV,IOPT)
      PARAMETER (NR1=25,NR2=17)
      CHARACTER*(*) CDEV,CR0*58,CR1(NR1)*58,CR2(NR2)*58
     ...
     *'Fortran 77 Demo of the Data Plotting Library DISLIN.      ',
     *'Author: H. Michels,  MPI fuer Sonnensystemforschung,      ',
     *'               D-39191 Katlenburg-Lindau, Germany         ',
     *'                                                          ',
     ...
     *'14: Map Plot                     30: Menue end            '/
      ...
      IF((IFIRST.EQ.0).OR.(IOPT.EQ.30)) THEN
        WRITE(6,'(/4X,3A)')	     '/****',CR0,   '****/'
...
C     Shading in the XZ plane
      ...
      CALL SURMSH('ON')
      STEP=2*PI/30.
      CALL SURFCP(ZFUN2,0.,2*PI,STEP,0.,2*PI,STEP)
      CALL DISFIN
      END

      FUNCTION ZFUN2(X,Y,IOPT)
     ...
     *               ZMAT(50,50)
      ...
      SUBROUTINE EX6_1
      CHARACTER*100 CT0,CT1,CT2,CT3,CTIT
...
C     INDIVIDUELLE SCHRAFFUREN AUSWAEHLEN
     ...
     *     IC2RAY/50,150,100,200,175/
      ...
      SUBROUTINE EXA_13
      PARAMETER (N=7)
      CHARACTER*4 CLR(N),CONT(N)
      DATA CLR/'WHIT','RED ','GREE','ORAN','YELL','CYAN','BLUE'/
     *    CONT/'ANTA','AFRI','EURA','NORT','SOUT','AUST','LAKE'/
      ...
      SUBROUTINE EXA_14
      CHARACTER*4 CVLT*8,CLIS*80
      DATA NB,NH,NCOL/100,80,0/

      CLIS='SMALL|RAINBOW|SPECTRUM|GREY|RRAIN|RGREY|VGA'

      CALL WGINI('VERT', IP)
      ILIS=2
      CALL WGLAB(IP, 'Colour Table:',ID)
      CALL WGLIS(IP,CLIS,ILIS,ID_LIS)
      CALL WGOK(IP,ID)
      CALL WGFIN

      CALL GWGLIS(ID_LIS,ILIS)
      CALL ITMSTR(CLIS,ILIS,CVLT)
