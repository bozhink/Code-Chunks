...
public abstract class AbstractTile extends Element {
  ...
  void draw(Graphics2D _g2, int _index) {
    ...
    if(getRealStyle().isDrawingFill()) { // First fill the inside
      _g2.setPaint(getPanel().projectColor(getRealStyle().getFillColor(), objects[_index].getDistance()));
      _g2.fillPolygon(a[_index], b[_index], sides);
    }
    if(getRealStyle().isDrawingLines()) {
      _g2.setStroke(getRealStyle().getLineStroke());
      _g2.setColor(getPanel().projectColor(getRealStyle().getLineColor(), objects[_index].getDistance()));
  ...
  protected void projectPoints() {
    for(int i = 0;i<numberOfTiles;i++) {
      ...
      for(int j = 0;j<sides;j++) {
        getPanel().project(corners[i][j], pixel); // Project each corner
        a[i][j] = (int) pixel[0];
        b[i][j] = (int) pixel[1];
        for(int k = 0;k<3;k++) {
      ...
      getPanel().project(center, pixel);                   // Project the center and take it
      objects[i].setDistance(pixel[2]*displacementFactor); // as reference for the distance
    }
    getPanel().project(getHotSpot(targetPosition), pixelOrigin);
