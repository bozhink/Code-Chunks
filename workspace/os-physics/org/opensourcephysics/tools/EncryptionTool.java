...
public class EncryptionTool extends OSPFrame implements Tool {
  ...
  public void refreshGUI() {
  	...
  	if (fileName != null) title += ": " + fileName; //$NON-NLS-1$
    setTitle(title);
    openButton.setToolTipText(ToolsRes.getString(
						"EncryptionTool.Button.Open.ToolTip")); //$NON-NLS-1$
    saveButton.setToolTipText(ToolsRes.getString(
						"EncryptionTool.Button.Save.ToolTip")); //$NON-NLS-1$
    passwordLabel.setText(ToolsRes.getString(
						"EncryptionTool.Label.Password")); //$NON-NLS-1$
    passwordField.setToolTipText(ToolsRes.getString(
						"EncryptionTool.PasswordField.ToolTip")); //$NON-NLS-1$
    encryptedCheckBox.setText(ToolsRes.getString(
    				"EncryptionTool.CheckBox.Encrypted")); //$NON-NLS-1$
    encryptedCheckBox.setToolTipText(ToolsRes.getString(
						"EncryptionTool.CheckBox.Encrypted.ToolTip")); //$NON-NLS-1$
    previewCheckBox.setText(ToolsRes.getString(
						"EncryptionTool.CheckBox.Preview")); //$NON-NLS-1$
    previewCheckBox.setToolTipText(ToolsRes.getString(
						"EncryptionTool.CheckBox.Preview.ToolTip")); //$NON-NLS-1$
    fileMenu.setText(ToolsRes.getString(
    				"EncryptionTool.Menu.File")); //$NON-NLS-1$
    openItem.setText(ToolsRes.getString(
    				"EncryptionTool.MenuItem.Open")); //$NON-NLS-1$
    saveItem.setText(ToolsRes.getString(
    				"EncryptionTool.MenuItem.Save")); //$NON-NLS-1$
    saveAsItem.setText(ToolsRes.getString(
    				"EncryptionTool.MenuItem.SaveAs")); //$NON-NLS-1$
    exitItem.setText(ToolsRes.getString(
    				"EncryptionTool.MenuItem.Exit")); //$NON-NLS-1$
    helpMenu.setText(ToolsRes.getString(
    				"EncryptionTool.Menu.Help")); //$NON-NLS-1$
    logItem.setText(ToolsRes.getString(
    				"EncryptionTool.MenuItem.Log")); //$NON-NLS-1$
    aboutItem.setText(ToolsRes.getString(
    				"EncryptionTool.MenuItem.About")); //$NON-NLS-1$
    ...
    encryptedCheckBox.setSelected(control != null &&
    				control.getPassword() != null);
    passwordLabel.setEnabled(encryptedCheckBox.isSelected());
    passwordField.setEnabled(encryptedCheckBox.isSelected());
    previewCheckBox.setEnabled(encryptedCheckBox.isEnabled() &&
    				encryptedCheckBox.isSelected());
    previewCheckBox.setSelected(control != null &&
    				control.getObjectClass() == Cryptic.class);
  }
...
// ______________________________ private methods _____________________________
  ...
  private void displayXML(XMLControlElement control) {
  	if (treePanel != null) contentPane.remove(treePanel);
...
//      treePanel.setSelectedNode("xml_password"); //$NON-NLS-1$
  ...
  private String save(String fileName) {
  	if (fileName == null || fileName.equals("")) return null; //$NON-NLS-1$
  ...
  private void createGUI() {
    ...
    aboutItem.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        String aboutString = toolname + VERSION + XML.NEW_LINE
        	+ToolsRes.getString("EncryptionTool.About.OSPName") + XML.NEW_LINE //$NON-NLS-1$
        	+"www.opensourcephysics.org"; //$NON-NLS-1$
        JOptionPane.showMessageDialog(EncryptionTool.this, 
        				aboutString,
								ToolsRes.getString("EncryptionTool.About.Title"), //$NON-NLS-1$
