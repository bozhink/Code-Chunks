...
public class JarTool implements Tool, Runnable {
  ...
  // --- End of localization
   ...
   private JarTool(){
      ...
      chooser = GUIUtils.createChooser("JAR, ZIP",new String[]{"zip","jar"});
...
// -----------------------------------
   ...
   public File create(ArrayList sources, File parent, File target, Manifest manifest) {
     ...
     try {
       ...
       String targetName = GUIUtils.chooseFilename(chooser);
       ...
       if (org.opensourcephysics.controls.XML.forwardSlash(target.getAbsolutePath()).equals(ResourceLoader.getLaunchJarPath())) {
...
//       return compressList(sources,parent,target,manifest,policy,ownerFrame);
     ...
     static public File extract(File source, String filename, File target) {
       if (source.exists()==false || filename==null || filename.trim().length()<=0 || target==null) return null;
       try {
         ZipFile input = new ZipFile(source);
         ZipEntry entry = input.getEntry(filename);
         if (entry==null) return null;
         // following code added by Douglas Brown June 2007
         // if entry is a folder, extract all contained folders and files
         if (entry.isDirectory()) {
           ...
           while ((zipEntry=zipInput.getNextEntry()) != null) {
             ...
             if (zipEntry.getName().startsWith(filename)) {
               ...
               File file = new File(target,zipEntry.getName().substring(n));
	             extract(source, zipEntry.getName(), file);
             }
           }
           return target;
         } // end of code added by Douglas Brown June 2007
      ...
      * @return boolean
      */
     static public boolean extract(Object source, AbstractList files, File targetDirectory) {
...
// -----------------------------------
  ...
  static private File compressList(ArrayList sources, File parent, File target, Manifest manifest,
    ...
    for (Iterator it=sources.iterator(); it.hasNext(); ) { // Copy or uncompress all sources to the temporary directory
      ...
      counter++;
      String filename = it.next().toString();
      if (filename!=null) errorMessage.append(processFile(filename,new File(parent,filename),temporaryDirectory,policy));
    }
  ...
  static private StringBuffer processFile (String filename, File file, File targetDirectory, OverwriteValue policy) {
    // following changed by Douglas Brown June 2007 to handle zip/jar entries
  	if (!file.exists() && filename.indexOf("!") == -1) {
  		return new StringBuffer(res.getString("JarTool.FileDoesntExist")+" " + file.getAbsolutePath()+".\n");
  	} // end of change by Douglas Brown June 2007
  	if (file.isDirectory()) { // recursively process each file in the directory
      StringBuffer errorMessage = new StringBuffer();
    ...
    // following code added by Douglas Brown June 2007 to handle zip/jar entries
    int n = filename.indexOf("!");
    if (n > - 1) {
    	String entry = filename.substring(n+2);
    	File zipFile = new File(filename.substring(0, n));
   ...
   * @param fsView FileSystemView
   */
  static private ArrayList recursiveGetDirectory (File directory, FileSystemView fsView) {
    File files[] = fsView.getFiles(directory, false);
    ArrayList list = new ArrayList();
    for (int i = 0; i < files.length; i++) {
