...
public class DatasetCurveFitter extends JPanel {
  ...
  public void fit() {
    ...
    for (int i = 0; i < x.length; i++) {
    	double next = fit.evaluate(x[i]);
    	double dev = (next-y[i]);
    	total += dev*dev;
    }
    rmsField.setValue(Math.sqrt(total/x.length));
      ...
      public void actionPerformed(ActionEvent e) {
        UserFunction f = (UserFunction)fit;
      	// display function builder
    		UserFunctionEditor builder = new UserFunctionEditor(f);
    		builder.setVisible(true);
    		if (builder.isApproved())
    			selectFit(f.getName());
    		else if (builder.isDeleted()) {
    	  	namedFits.remove(f.getName());
    			fitDropDown.removeItem(f.getName());
    			fitDropDown.setSelectedIndex(0);
  ...
  protected void createFit(String name) {
    ...
    fit = f;
		f.setExpression("0", dataset.getColumnName(0)); //$NON-NLS-1$
    // assemble display
    toolbar.removeAll();
    toolbar.add(fitDropDown);
    toolbar.addSeparator();
  	...
  	// display function builder
		...
		if (builder.isApproved()) {
	  	namedFits.put(name, f);
			...
			fitDropDown.setSelectedIndex(prevFit);
			selectFit((String)fitDropDown.getItemAt(prevFit));
		}
  }
