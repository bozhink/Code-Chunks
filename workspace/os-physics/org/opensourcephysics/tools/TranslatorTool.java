...
public class TranslatorTool extends OSPFrame implements Tool {
   ...
   private static Map defaultProps = new HashMap(); // class to default names/values
   private static Map classes = new HashMap();    // class to language map
   private static Map associates = new HashMap(); // object to class
   private static Set changed = new HashSet();    // contains maps with unsaved changes
   private static Locale locale = Locale.getDefault();
   private static Set searched = new HashSet(); // classes searched for translations
    ...
    */
   public void send(Job job, Tool replyTo) {/** not implemented */}
   ...
   public void setLocale(Locale locale) {
      ...
      if(localeDropDown!=null) {
         ...
         Map properties = getProperties(classType, locale);
    ...
    */
   public static synchronized void associate(Object obj, Class type) {
      if(obj==null) return;
   ...
   public void showProperties(Class type) {
      ...
      Collection names = control.getPropertyNames();
      ...
      Map properties = getProperties(type, locale);
      ...
      // compare with defaults and flag unused properties
    	Set keys = getDefaults(type).keySet();
      it = properties.keySet().iterator();
      while(it.hasNext()) {
        String key = (String)it.next();
   ...
   public static String getProperty(Class type, String key) {
      return getProperty(type, key, key, ToolsRes.locale);
   }
   ...
   public static String getProperty(Class type, String key, String defaultValue) {
      return getProperty(type, key, defaultValue, ToolsRes.locale);
   }
   ...
   public static String getProperty(Object obj, String key, String defaultValue) {
      if(obj==null) return (defaultValue==null) ? key : defaultValue;
    ...
    */
   public static Collection getAssociates(Class type) {
   ...
   public static Locale[] getTranslatedLocales(Class type) {
       ...
       Set langs = locales.keySet(); // loaded language codes
  		 String path = getPath(type);
  		 Resource res = null;
  		 String[] languages = Locale.getISOLanguages(); // all standard language codes
  	   for (int i = 0; i < languages.length; i++) {
  	  	 if (langs.contains(languages[i])) continue;
  	  	 res = ResourceLoader.getResource(
  	  					 path + "_" + languages[i] + ".properties"); //$NON-NLS-1$ //$NON-NLS-2$
  			 if (res != null) {
     ...
     languages.addAll(((Map)classes.get(type)).keySet()); // alphabetize
   ...
   protected static boolean isLanguage(String lang) {
		 String[] languages = Locale.getISOLanguages(); // all standard language codes
	   for (int i = 0; i < languages.length; i++) {
	  	 if (languages[i].equals(lang)) return true;
		 }
     return false;
   }
   ...
   private void addDropDownItem(LocaleItem item) {
     ...
     for(int i = 0; i<localeDropDown.getItemCount(); i++) {
    	 ...
    	 if (next.isDefault()) 
    		 defaultItem = next;
    	 else items.put(next.language.toLowerCase(), next);
     }
     localeDropDown.removeAllItems();
     if (defaultItem != null) localeDropDown.addItem(defaultItem);
   ...
   private static Map getDefaults(Class type) {
      Map defaults = (Map)defaultProps.get(type);
      if(defaults==null) {
   ...
   private static Map getProperties(Class type, Locale locale) {
      // look for properties map by class and language
      Map locales = (Map) classes.get(type);
      if(locales==null) {
         locales = new HashMap();
      	 synchronized(classes) {
      ...
      Map properties = (Map) locales.get(locale.getLanguage());
      if(properties==null) {
   ...
   private void createGUI() {
      ...
      table.addMouseListener(new MouseAdapter() {
        public void mousePressed(MouseEvent e) {
          if(e.isPopupTrigger()||e.getButton()==MouseEvent.BUTTON3||(e.isControlDown()&&System.getProperty("os.name").indexOf("Mac")>-1)) { //$NON-NLS-1$ //$NON-NLS-2$
          	// find clicked row
          	for (int i = 0; i < table.getRowCount(); i++) {
                ...
                // show popup menu
              	...
              	JMenuItem removeItem = new JMenuItem(
              					ToolsRes.getString("TranslatorTool.Popup.MenuItem.Remove")  //$NON-NLS-1$
   ...
   protected static void refreshAssociates(Class type) {
  		Iterator it = getAssociates(type).iterator();
  		while (it.hasNext()) {
  			Object obj = it.next();
  			if (obj instanceof XMLTable) {
  					...
  					((OSPControl)obj).propertyChange(
  							new java.beans.PropertyChangeEvent(TOOL, "translation", null, null)); //$NON-NLS-1$
  			}
  		}
   }
   
   private static synchronized void flagChange(Map properties) {
  	 synchronized(changed) {
   ...
   private class LocaleItem {
      ...
      public boolean isDefault() {
      	return loc.getDisplayLanguage(loc).equals(
      					Locale.getDefault().getDisplayLanguage(loc));
      }
   }
}
/*
