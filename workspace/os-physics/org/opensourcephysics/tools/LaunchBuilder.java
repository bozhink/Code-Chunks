...
//    javax.swing.JFrame.setDefaultLookAndFeelDecorated(true);
  ...
  public String save(LaunchNode node, String fileName) {
    ...
    if(!control.canWrite) {
      ...
      JOptionPane.showMessageDialog(null,
      				LaunchRes.getString("Dialog.SaveFailed.Message")+" "+fileName, //$NON-NLS-1$ //$NON-NLS-2$
  ...
  public String saveAs(LaunchNode node) {
    ...
    int result = getXMLChooser().showDialog(null,
    				LaunchRes.getString("FileChooser.SaveAs.Title")); //$NON-NLS-1$
      ...
      if(file.exists()) {
        ...
        if(getOpenPaths().contains(name)) {
          JOptionPane.showMessageDialog(frame,
          				LaunchRes.getString("Dialog.DuplicateFileName.Message")+" \""+name+"\"", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
          				LaunchRes.getString("Dialog.DuplicateFileName.Title"), //$NON-NLS-1$
          				JOptionPane.WARNING_MESSAGE);
          return null;
        }
        int selected = JOptionPane.showConfirmDialog(frame,
        				LaunchRes.getString("Dialog.ReplaceFile.Message")+" "+ //$NON-NLS-1$ //$NON-NLS-2$
        				file.getName()+XML.NEW_LINE+
        				LaunchRes.getString("Dialog.ReplaceFile.Question"), //$NON-NLS-1$
  ...
  protected void refreshNode(LaunchNode node) {
    ...
    if(node!=null) {
      ...
      if((jarPath.equals("")&&node.classPath!=null)||(!jarPath.equals("")&&!jarPath.equals(node.classPath))) { //$NON-NLS-1$ //$NON-NLS-2$
        ...
        node.setClassPath(jarPath.equals("") ? null : jarPath); //$NON-NLS-1$
        changed = true;
      }
      n = ((Integer) htmlSpinner.getValue()).intValue();
      LaunchNode.HTML html = node.getHTML(n);
      String input = urlField.getText();
      if(input.equals("")) input = null; //$NON-NLS-1$
      if(html == null || ((html.path!=null&&!html.path.equals(input))||
      				(input!=null&&!input.equals(html.path)))) {
    	String title = html == null? null: html.title;
        node.setHTML(n, title, input);
        changed = true;
      }
      input = htmlTabTitleField.getText();
      if(input.equals("")) input = null; //$NON-NLS-1$
      if(html != null && ((html.title!=null&&!html.title.equals(input))||
      				(input!=null&&!input.equals(html.title)))) {
        OSPLog.finest(LaunchRes.getString("Log.Message.ChangeNodeHTMLTabTitle")); //$NON-NLS-1$
        node.setHTML(n, input, html.path);
        changed = true;
      }
  ...
  protected boolean removeSelectedTab() {
    ...
    if(tabbedPane.getTabCount()==1) {
      LinkEdit edit = null;
      if (tabSetName != null && new File(tabSetName).exists()) {
        String[] args = new String[] {" "}; //$NON-NLS-1$
        edit = new LinkEdit(args);
      }
      boolean allRemoved = removeAllTabs();
			if (allRemoved && edit != null) {
  ...
  protected boolean saveChanges(LaunchPanel tab) {
    ...
    if(changed) {
      int selected = JOptionPane.showConfirmDialog(frame,
      				LaunchRes.getString("Dialog.SaveChanges.Tab.Message")+" \""+ //$NON-NLS-1$ //$NON-NLS-2$
  ...
  protected void refreshStringResources() {
    ...
    htmlTitle.setTitle(LaunchRes.getString("Label.HTML")); //$NON-NLS-1$
    ...
    htmlLabel.setText(LaunchRes.getString("Label.HTMLSpinner")); //$NON-NLS-1$
    htmlTabTitleLabel.setText(LaunchRes.getString("Label.HTMLTabTitle")); //$NON-NLS-1$
    urlLabel.setText(LaunchRes.getString("Label.URL")); //$NON-NLS-1$
  ...
  protected void refreshGUI() {
    ...
    if(newNodeSelected) {
      ...
      htmlSpinner.setValue(new Integer(0));
    ...
    if(node!=null) {
      ...
      int n = ((Integer) htmlSpinner.getValue()).intValue();
      LaunchNode.HTML html = node.getHTML(n);
      String urlPath = html!= null? html.path: null;
      urlField.setText(urlPath);
      urlField.setBackground(urlPath!=null&&html.url==null? RED : Color.white);
      htmlSpinnerModel.setMaximum(new Integer(node.getHTMLCount()));
      htmlSpinner.setVisible(node.getHTML(0) != null);
      boolean hasURL = html!= null && html.path != null;
      htmlTabTitleLabel.setVisible(hasURL);
      htmlTabTitleField.setVisible(hasURL);
      htmlTabTitleField.setText(html!= null? html.title: null);
      htmlTabTitleField.setBackground(Color.white);
      if (html != null && html.url!=null && htmlPane.getPage() != html.url) {
      	try {
      ...
      else argField.setBackground(Color.white);
  ...
  protected void createGUI(boolean splash) {
   ...
   super.createGUI(splash);
    ...
    htmlTitle = BorderFactory.createTitledBorder(LaunchRes.getString("Label.HTML")); //$NON-NLS-1$
    ...
    htmlTabTitleField = new JTextField();
    htmlTabTitleField.addKeyListener(keyListener);
    htmlTabTitleField.addFocusListener(focusListener);
    ...
    htmlScroller.setBorder(htmlTitle);
    ...
    urlLabel = new JLabel();
    urlLabel.setBorder(BorderFactory.createEmptyBorder(0, 2, 0, 2));
    htmlTabTitleLabel = new JLabel();
    htmlTabTitleLabel.setBorder(BorderFactory.createEmptyBorder(0, 2, 0, 2));
    htmlSpinnerModel = new SpinnerNumberModel(0, 0, 1, 1);
    htmlSpinner = new JSpinner(htmlSpinnerModel);
    JSpinner.NumberEditor editor = new JSpinner.NumberEditor(htmlSpinner);
    htmlSpinner.setEditor(editor);
    htmlSpinner.addChangeListener(new ChangeListener() {
      public void stateChanged(ChangeEvent e) {
        if(urlField.getBackground()==Color.yellow) {
          refreshSelectedNode();
        } else {
          refreshGUI();
        }
    ...
    urlBar.add(htmlLabel);
    urlBar.add(htmlSpinner);
    urlBar.add(urlLabel);
    urlBar.add(urlField);
    urlBar.add(openURLAction);
    urlBar.add(htmlTabTitleLabel);
    urlBar.add(htmlTabTitleField);
  ...
  protected void setFontLevel(int level) {
  	...
  	// set font levels of titled borders
		FontSizer.setFonts(htmlTitle, level);
  ...
  protected void createActions() {
    ...
    openJarAction = new AbstractAction(null, openIcon) {
      public void actionPerformed(ActionEvent e) {
        ...
        if(result==JFileChooser.APPROVE_OPTION) {
          ...
          if (jars.indexOf(newJar) > -1) newJar = null;
          if (!jars.equals("")) jars += ";"; //$NON-NLS-1$ //$NON-NLS-2$
          if (newJar != null) jarField.setText(jars + newJar);
    ...
    saveJarAction = new AbstractAction(LaunchRes.getString("LaunchBuilder.Action.CreateJar.Name")) { //$NON-NLS-1$
      public void actionPerformed(ActionEvent e) {
        // define base folder
        File base = new File(XML.getDirectoryPath(ResourceLoader.launchJarPath));
				String jarName = XML.stripExtension(LaunchBuilder.this.tabSetName)
								+ ".jar"; //$NON-NLS-1$
				...
				// in jarChooser, preselect launch jar and all non-zip child files
				source.add(ResourceLoader.launchJarName); // launch jar added first
				// add files in accept method
				base.listFiles(new java.io.FileFilter() {
					public boolean accept(File file) {
						// skip jar and zip files
						if (file.getName().endsWith(".jar") || //$NON-NLS-1$
										file.getName().endsWith(".zip")) //$NON-NLS-1$
						return false;
				...
				jarChooser.setSelectionRelativePaths((String[])source.toArray(new String[0]));
				...
				for (int i = 0; i < paths.length; i++) {
					if (paths[i].equals(ResourceLoader.launchJarName))
						// put launch jar first so later paths will overwrite
						source.add(0, paths[i]);
					else
						source.add(paths[i]);
				}
				java.util.jar.Manifest manifest = JarTool.createManifest(
								"", "org.opensourcephysics.tools.Launcher"); //$NON-NLS-1$ //$NON-NLS-2$
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if (tabSetName != null && new File(tabSetName).exists()) {
          ...
          edit = new LinkEdit(args);
				...
				if (removeAllTabs()) {
					if (edit != null) undoSupport.postEdit(edit);
    ...
    copyAction = new AbstractAction() {
      public void actionPerformed(ActionEvent e) {
        ...
        if(node!=null) {
          XMLControl control = new XMLControlElement(node);
          control.setValue("filename", node.getFileName()); //$NON-NLS-1$
        ...
        try {
          ...
          if(dataString!=null) {
            ...
            if(control.getObjectClass()==LaunchNode.class) {
              String fileName = control.getString("filename"); //$NON-NLS-1$
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if(result==JFileChooser.APPROVE_OPTION) {
          ...
          if(control.failedToRead()) {
            ...
            JOptionPane.showMessageDialog(null, LaunchRes.getString("Dialog.InvalidXML.Message")+" \""+XML.getName(fileName)+"\"", LaunchRes.getString("Dialog.InvalidXML.Title"), JOptionPane.WARNING_MESSAGE); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            return;
          }
          if(control.getObjectClass()==LaunchNode.class) {
            // add the child node
            ...
            JOptionPane.showMessageDialog(null, LaunchRes.getString("Dialog.NotLauncherFile.Message")+" \""+XML.getName(fileName)+"\"", LaunchRes.getString("Dialog.NotLauncherFile.Title"), JOptionPane.WARNING_MESSAGE); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          }
        }
      }
    };
  ...
  protected void addChildToSelectedNode(LaunchNode child) {
    ...
    if(parent!=null&&child!=null) {
      ...
      for(int i = 0;i<nodes.length;i++) {
        ...
        if(node!=null) {
          getSelectedTab().setSelectedNode(node);
          JOptionPane.showMessageDialog(frame, LaunchRes.getString("Dialog.DuplicateNode.Message")+" \""+node+"\"", LaunchRes.getString("Dialog.DuplicateNode.Title"), JOptionPane.WARNING_MESSAGE); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
          return;
        }
      }
  ...
  protected void showAboutDialog() {
    ...
    JOptionPane.showMessageDialog(frame, aboutString,
        LaunchRes.getString("Help.About.Title")+" LaunchBuilder", //$NON-NLS-1$ //$NON-NLS-2$
