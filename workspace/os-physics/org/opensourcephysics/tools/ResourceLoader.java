...
public class ResourceLoader {
   ...
   protected static Map zipLoaders = new TreeMap();          // maps zip to zipLoader
   ...
   static private Resource createURLResource(String path) {
      ...
      // else if applet mode and relative path, search document and code base
         ...
         if((OSPFrame.applet!=null)&&!path.startsWith("/")) { //$NON-NLS-1$
   ...
   private static String getPath(String base, String name) {
      ...
      if((System.getProperty("os.name").indexOf("Mac")>-1)&&path.startsWith("file:/")&&!path.startsWith("file:///")) { //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
