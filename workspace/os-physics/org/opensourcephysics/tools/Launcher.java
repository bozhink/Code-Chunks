...
public class Launcher {
   ...
   protected static String version = "1.3";                                           //$NON-NLS-1$
   protected static String releaseDate = "September 2006";                            //$NON-NLS-1$
   ...
   protected static HyperlinkListener linkListener;
   ...
   public Launcher(boolean splash) {
      ...
      if(OSPFrame.applet==null) {                                   // never allow new VM if Launcher was instantiated by an applet
         ...
         Runnable launchRunner = new Runnable() {
            ...
            public void run() {
               // prevent single VM in Linux and Vista
               if(System.getProperty("os.name", "")                 //$NON-NLS-1$ //$NON-NLS-2$
                   .toLowerCase().indexOf("vista")>-1) {
                  return;                                           //$NON-NLS-1$
               }
               if(System.getProperty("os.name", "")                 //$NON-NLS-1$ //$NON-NLS-2$
   ...
   public Launcher(String fileName, boolean splash) {
      ...
      if(fileName==null) {
         ...
         if(ResourceLoader.launchJarName!=null) {
            fileName = XML.stripExtension(ResourceLoader.launchJarName)+".xset"; //$NON-NLS-1$
            path = open(fileName);
         }
         if(path==null) {
   ...
   public LaunchNode setSelectedNodeByKey(String keywords, int pageNumber) {
      ...
      for(int i = 0; i<getTabCount(); i++) {
         ...
         while(e.hasMoreElements()) {
            LaunchNode node = (LaunchNode) e.nextElement();
            if(keywords.equals(node.getKeywords())) { // found node
               setSelectedTab(tab);
    ...
    * @return the html tab
    */
   public HTMLTab getHTMLTab(int i) {
      while(i>=htmlTabList.size()) {
         htmlTabList.add(new HTMLTab());
      }
      return(HTMLTab) htmlTabList.get(i);
   }
   ...
   public String open(String name) {
      ...
      if(zip>-1) {
         ...
         jarBasePath = s.equals("") ? XML.forwardSlash(System.getProperty("user.dir", "")) : s; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      ...
      if(control.getPassword()!=null) {
         ...
         if((Launcher.this instanceof LaunchBuilder)||pwRequired) {
            if(Password.verify(control.getPassword(), returnPath)) {
               ...
               if(undoEdit!=null) {
                  undoSupport.postEdit(new LinkEdit(undoEdit));
                  undoEdit = null;
               }
               return null;
            }
      ...
      if(LaunchSet.class.equals(type)) {
         ...
         if(!returnPath.equals("")&&(splashDialog!=null)&&splashDialog.isVisible()) {               //$NON-NLS-1$
            ...
            isWebStart = res.getAbsolutePath().indexOf("javaws")>-1;                                //$NON-NLS-1$
            if(res.getFile()!=null) {                                                               // external file
               ...
               boolean internal = jarName.equals(LaunchRes.getString("Splash.Label.Internal"))||    //$NON-NLS-1$
                                 jarName.equals(ResourceLoader.launchJarName);
               if(internal) {
      ...
      ActionListener openAction = new AbstractAction() {
         ...
         public void actionPerformed(ActionEvent e) {
            ...
            if(child.getLaunchClass()==null) {
               if(tabSetName!=null) {
                  UndoableEdit edit = new LinkEdit(child.args);
   ...
   protected void showTabbedPaneView() {
      if(contentPane.getComponent(0)==tabbedPane) {
         return;
      }
      contentPane.removeAll();
   ...
   protected void refreshStringResources() {
      ...
      aboutItem.setText(LaunchRes.getString("Menu.Help.About"));            //$NON-NLS-1$
   ...
   protected void createGUI(boolean splash) {
      ...
      // instantiate the OSPLog and TranslatorTool
      OSPLog.getOSPLog();
      ...
      tabbedPane.addChangeListener(new ChangeListener() {
         ...
         public void stateChanged(ChangeEvent e) {
            ...
            if((root!=null)&&root.isButtonView()&&(Launcher.this.getClass()==Launcher.class)) {
               ...
               LaunchPanel tab = getSelectedTab();
               if(tab!=null) {
                  tab.displayHTML(tab.getSelectedNode());
               }
            }
            previousNode = selectedNode;
            newNodeSelected = true;
      ...
      lookFeelItem.setSelected(!OSPFrame.DEFAULT_LOOK_AND_FEEL);
      ...
      logItem.addActionListener(new ActionListener() {
         ...
         public void actionPerformed(ActionEvent e) {
            if(OSPFrame.applet==null) { // not running as applet so create and position the log.
      ...
      if(ResourceLoader.launchJarPath!=null) {
         ...
         JarFile jar = null;
         try {
            if(OSPFrame.applet==null) {                                                   // application mode
               jar = new JarFile(ResourceLoader.launchJarPath);
            } else {                                                                      // applet mode
               URL url;
               if(ResourceLoader.launchJarPath.startsWith("http:")) {                     //$NON-NLS-1$
                  // create a URL that refers to a jar file on the web
                  url = new URL("jar:"+ResourceLoader.launchJarPath+"!/");                //$NON-NLS-1$ //$NON-NLS-2$
               } else {
                  // create a URL that refers to a local jar file
                  url = new URL("jar:file:/"+ResourceLoader.launchJarPath+"!/");          //$NON-NLS-1$ //$NON-NLS-2$
               }
               // get the jar
            ...
            OSPLog.warning(ex.getClass().getName()+": "+ex.getMessage());                 //$NON-NLS-1$
         ...
         if(jar!=null) {
            Action action = new AbstractAction() {
               ...
               public void actionPerformed(ActionEvent e) {
                  ...
                  fileName = ResourceLoader.launchJarName+"!/"+fileName;                  //$NON-NLS-1$
                  if(tabSetName!=null) {
                     UndoableEdit edit = new LinkEdit(new String[]{fileName});
    ...
    *
    * @param defaultLF true to use default L&F
    */
    ...
    */
   protected static void setLinksEnabled(JEditorPane textPane, boolean enabled) {
            ...
            public void hyperlinkUpdate(HyperlinkEvent e) {
               if(e.getEventType()==HyperlinkEvent.EventType.ACTIVATED) {
                  try {
                     ...
                     if(textPane instanceof HTMLTab.HTMLPane) {
                        HTMLTab tab = ((HTMLTab.HTMLPane) textPane).tab;
                        URL prev = textPane.getPage();
                        if(prev!=e.getURL()) {
                           tab.previousPages.push(prev);
   ...
   protected void handleMousePressed(MouseEvent e, final LaunchPanel tab) {
      ...
      if(e.isPopupTrigger()||(e.getButton()==MouseEvent.BUTTON3)
         ...
         if(tabSetName!=null) {
            UndoableEdit edit = new LinkEdit(selectedNode.args);
            if(open(selectedNode.args)!=null) {
               undoSupport.postEdit(edit);
               refreshGUI();
            } else {
               open(selectedNode.args);
               refreshGUI();
            }
         }
         ...
         for(int i = 0; i<jars.length; i++) {
            if(!jarList.equals("")) {                                                                           //$NON-NLS-1$
               if(jars[i].equals(ResourceLoader.launchJarName)) {
                  continue;
               }
   ...
   private void splash() {
      ...
      if(splashDialog==null) {
         ...
         JLabel creditsLabel = new JLabel("By Douglas Brown"); //$NON-NLS-1$
         creditsLabel.setFont(font.deriveFont(10f));
         ...
         splashPathLabel.setFont(font.deriveFont(12f));
   ...
   protected void exit() {
      ...
      if((OSPFrame.applet==null)&&(frame.getDefaultCloseOperation()==WindowConstants.HIDE_ON_CLOSE)) {
         System.exit(0);
      }
            ...
            while(e.hasMoreElements()) {
               ...
               if(!node.processes.isEmpty()) {
                  if(!approved&&!declined) {                                                                        // ask for approval
                     int selected = JOptionPane.showConfirmDialog(frame,
                        LaunchRes.getString("Dialog.TerminateSeparateVM.Message")+XML.NEW_LINE                      //$NON-NLS-1$
                        +LaunchRes.getString("Dialog.TerminateSeparateVM.Question"), LaunchRes.getString(           //$NON-NLS-1$
                           "Dialog.TerminateSeparateVM.Title"), JOptionPane.YES_NO_OPTION);                         //$NON-NLS-1$
   ...
   private class LaunchRenderer extends DefaultTreeCellRenderer {
      ...
      public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded,
         ...
         setToolTipText(node.tooltip.equals("") ? null : node.tooltip); //$NON-NLS-1$
   ...
   public class LaunchSet implements XML.ObjectLoader {
      ...
      public void saveObject(XMLControl control, Object obj) {
         ...
         control.setValue("default_look_and_feel", OSPFrame.DEFAULT_LOOK_AND_FEEL); //$NON-NLS-1$
         ...
         if((nodes!=null)&&!nodes.isEmpty()) {
            ...
            if(buttonNode!=null) {
               for(int j = 0; j<tabbedPane.getTabCount(); j++) {
                  if(getTab(j).getRootNode()==buttonNode) {
   ...
   protected class LinkEdit extends AbstractUndoableEdit {
       ...
       * Constructor.
       *
       * @param arguments [0] is new file name, [1] is new tab and node name
       */
      public LinkEdit(String[] arguments) {
         args[0] = arguments[0];
         args[1] = (arguments.length<2) ? "" : arguments[1]; //$NON-NLS-1$
         // prepend launch jar name to internal xsets
         String fileName = tabSetName;
         if(!fileName.startsWith(defaultFileName)&&tabSetBasePath.equals("")) { //$NON-NLS-1$
            fileName = ResourceLoader.launchJarName+"!/"+fileName;              //$NON-NLS-1$
         }
         prev[0] = args[0].equals("") ? "" : fileName; //$NON-NLS-1$ //$NON-NLS-2$
         if(getSelectedNode()!=null) {
            prev[1] = getSelectedNode().getPathString();
         } else {
            prev[1] = (getSelectedTab()==null) ? "" : getSelectedTab().getRootNode().name; //$NON-NLS-1$
         }
      }
       ...
       * Copy constructor.
       *
       * @param edit the LinkEdit to copy
       */
   ...
   public static void main(String[] args) {
      ...
      if((args!=null)&&(args.length>0)) {
         ...
         // look for default file with launchJarName or defaultFileName
         if(ResourceLoader.launchJarName!=null) {
            path = launcher.open(XML.stripExtension(ResourceLoader.launchJarName)+".xset"); //$NON-NLS-1$
         }
         if(path==null) {
   ...
   private static void findFramesFor(final LaunchNode node, Frame[] prevFrames, final Runnable runner) {
      ...
      for(int i = 0; i<frames.length; i++) {
         if(frames[i] instanceof JFrame) {                                    // found new frame
            JFrame frame = (JFrame) frames[i];
            if(((frame instanceof OSPFrame)&&((OSPFrame) frame).wishesToExit())
            ...
            public void mousePressed(MouseEvent e) {
               ...
               if(e.isPopupTrigger()||(e.getButton()==MouseEvent.BUTTON3)
                  ...
                  item.addActionListener(new ActionListener() {
                     ...
                     public void actionPerformed(ActionEvent e) {
                        try {
                           URL page = (URL) previousPages.pop();
