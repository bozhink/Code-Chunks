...
public class CheckTreeSelectionModel extends DefaultTreeSelectionModel { 
  ...
  public CheckTreeSelectionModel(TreeModel model) {
	 ...
	 */
	public boolean isUnselected(TreePath path) {
		if (isAncestorSelected(path)) return false; // is fully selected
		TreePath[] selectionPaths = getSelectionPaths();
		if (selectionPaths == null) return true; // nothing is selected
	 ...
	 * @param path the path to test
	 */
	public boolean isAncestorSelected(TreePath path) {
	...
	public void addSelectionPaths(TreePath[] paths) {
		...
		for (int i = 0; i < paths.length; i++) {
			...
			TreePath[] selectionPaths = getSelectionPaths();
			if (selectionPaths == null) break; // nothing to unselect
	...
	public void removeSelectionPaths(TreePath[] paths) {
		TreePath[] prev = getSelectionPaths();
		if (prev == null) return; // nothing to remove
		for (int i = 0; i < paths.length; i++) {
			TreePath path = paths[i];
