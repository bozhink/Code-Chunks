 ...
 */
  protected void addMenuItems() {
    ...
    // log scale
    menu.addSeparator();
    logItem = new JCheckBoxMenuItem("Log Scale", false);
...
protected JMenu loadToolsMenu() {
  ...
  datasetItem.addActionListener(new ActionListener() {
    public void actionPerformed(ActionEvent e) {
        double[][] data= (histogram.logScale)? getLogPoints(): getPoints();
        double[] bins = data[0];
        double[] vals = data[1];
        double start=bins[0]-histogram.getBinWidth()/2;
        double stop=bins[bins.length-1]+histogram.getBinWidth()/2;
        if(tool==null || !tool.isDisplayable()){
          dataset = new HistogramDataset(start,stop,histogram.getBinWidth());
          dataset.append(bins, vals);
          tool = new DatasetTool(dataset);
        }else{
          dataset.clear();
          dataset.setBinWidth(start,stop,histogram.getBinWidth());
          dataset.append(bins, vals);
          tool.updateData(dataset);
        }
