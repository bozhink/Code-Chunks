...
public class DrawingPanel extends JPanel implements Printable, ActionListener, Renderable {
   ...
   protected JMenuItem propertiesItem,scaleItem,zoomInItem,zoomOutItem,snapshotItem;                                         // the menu item for the properites dialog box
   ...
   protected double xMarginPercentage=0.0, yMarginPercentage=0.0; // Added by Paco
    ...
    * @param _percentage
    */
   public void setXMarginPercentage(double _percentage) { // Added by Paco
      xMarginPercentage = _percentage;
   }
    ...
    * @param _percentage
    */
   public void setYMarginPercentage(double _percentage) { // Added by Paco
      yMarginPercentage = _percentage;
   }
    ...
    * @return the image buffer
    */
   public synchronized BufferedImage render(BufferedImage image) {
   ...
   /**
    * Computes the size of the gutters. Objects, such as axes, perform this function
      ...
      if (dimensionSetter!=null){
         ...
         if (interiorDimension!=null){
            ...
            leftGutter = rightGutter =
               Math.max(0, getWidth()-interiorDimension.width)/2;
            topGutter = bottomGutter =
               Math.max(0, getHeight()-interiorDimension.height)/2;
         }
      }
   ...
   protected void paintEverything(Graphics g) {
      ...
      ArrayList tempList = getDrawables(); // holds a clone of the drawable object list
      ...
      if(antialiasTextOn) ((Graphics2D)g).setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
        else ((Graphics2D)g).setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,RenderingHints.VALUE_TEXT_ANTIALIAS_OFF);
      if(antialiasShapeOn) ((Graphics2D)g).setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
   ...
   protected void zoomIn() {
      setMessage(DisplayRes.getString("DisplayPanel.Zoom_msg"),DrawingPanel.BOTTOM_LEFT); //$NON-NLS-1$
      zoomMode = true;
   }
   ...
   public double getAspectRatio() {
      return(pixelMatrix[3]==1)
            ? 1
            : Math.abs(pixelMatrix[0]/pixelMatrix[3]);
   }
   ...
   public Rectangle2D getMeasure() {
      ...
      while(it.hasNext()) {
         ...
         if(!Double.isNaN(measurable.getXMax())&&!Double.isNaN(measurable.getXMin())
             &&!Double.isNaN(measurable.getYMax())&&!Double.isNaN(measurable.getYMin())) {
    ...
    * Sets the x axis scale based on the max and min values of all measurable objects.  Autoscale flag is not respected.
    */
   void scaleX(ArrayList tempList) {
      ...
      if(measurableFound) {
        if (xMarginPercentage>0.0) { // Added by Paco
          double xMed = (newXMin+newXMax)/2, xLen = (newXMax-newXMin)/2;
          newXMax = xMed + xLen*(1.0+xMarginPercentage/100.0);
          newXMin = xMed - xLen*(1.0+xMarginPercentage/100.0);
        }
    ...
    * Sets the y axis scale based on the max and min values of all measurable objects. Autoscale flag is not respected.
    */
   void scaleY(ArrayList tempList) {
      ...
      // do not change change values unless there is at least one measurable object.
      if(measurableFound) {
        if (yMarginPercentage>0.0) { // Added by Paco
          double yMed = (newYMin+newYMax)/2, yLen = (newYMax-newYMin)/2;
          newYMax = yMed + yLen*(1.0+yMarginPercentage/100.0);
          newYMin = yMed - yLen*(1.0+yMarginPercentage/100.0);
        }
   ...
   public class ZoomBox {
      ...
      public Rectangle2D.Double reportZoom(int xpix, int ypix) {
         ...
         setMessage(null,DrawingPanel.BOTTOM_LEFT);
         if(!visible) {
            return null; // startZoom must be called first
         }
         xstop = xpix;
         ystop = ypix;
         visible = false;
         if((xstart==xstop)||(ystart==ystop)) {
            return null; // the user clicked but did not drag
         }
         double xmin = pixToX(xstart);
         double xmax = pixToX(xstop);
         double ymax = pixToY(ystart);
