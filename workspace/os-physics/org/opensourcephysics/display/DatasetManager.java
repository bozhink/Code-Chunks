...
public class DatasetManager extends AbstractTableModel implements Measurable {
  ...
  static Color[] lineColors = {
    Color.red, Color.green, Color.blue, Color.yellow.darker(), Color.cyan, Color.magenta
  };
 ...
 */
  ...
  public int addDataset(Dataset dataset) {
    if(linked) dataset.setXColumnVisible(false);
  ...
  public Dataset removeDataset(int index) {
    if (index < 0 || index > datasets.size() - 1) return null;
    return (Dataset)datasets.remove(index);
  }
  ...
  public int getDatasetIndex(String yColumnName) {
  	for (int i = 0; i < datasets.size(); i++) {
  		String name = ((Dataset)datasets.get(i)).getYColumnName();
      if (name.equals(yColumnName)) return i;
  	}
  	return -1;
  }
