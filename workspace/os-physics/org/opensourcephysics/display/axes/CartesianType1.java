...
public class CartesianType1 extends AbstractAxes implements CartesianAxes, Dimensioned {
  ...
  protected DrawableTextLine xLine = new DrawableTextLine("x", 0, 0);
  protected DrawableTextLine yLine = new DrawableTextLine("y", 0, 0);

  /** If XTicks or YTicks are given/ */
  ArrayList xticks = null, xticklabels = null, yticks = null, yticklabels = null;
  ...
  /** gutters for plot. The initial values are the default values */
  int topGutter = 25;
  int bottomGutter = 45;
  int leftGutter = 45;
  int rightGutter = 25;
  ...
  /** default gutters. */
  int defaultTopGutter = topGutter;
  int defaultBottomGutter = bottomGutter;
  int defaultLeftGutter = leftGutter;
  int defaultRightGutter = rightGutter;
  ...
  public CartesianType1(PlottingPanel panel) {
    ...
    superscriptFont = new Font("Dialog", Font.PLAIN, 9);
    xLine.setJustification(DrawableTextLine.CENTER);
    xLine.setFont(labelFont);
    xLine.setPixelXY(true);
    yLine.setJustification(DrawableTextLine.CENTER);
  ...
  private int getLeftGutter(DrawingPanel panel) {
    ...
    double yTickSize = roundUp((ytickMax-ytickMin)/(double) numberYTickMarks);
    ...
    for(int i = 0;i<=numberYTickMarks;i++) {
      ...
      gutter = Math.max(labelFontMetrics.stringWidth(yticklabel)+25, gutter);
      ypos += yTickSize;
    }
  ...
  public void draw(DrawingPanel panel, Graphics g) {
    ...
    if(adjustGutters && panel.isAutoscaleY()) {
      leftGutter = getLeftGutter(panel);
      panel.setGutters(leftGutter, topGutter, rightGutter, bottomGutter);
    }
    drawPlot(panel, g);
  }
  ...
  protected void drawPlot(DrawingPanel panel, Graphics graphics) {
    ...
    if((xExponent!=0)&&(xticks==null)) {
      ...
      if(!xlog) {
        ...
        xStartPosition -= labelFontMetrics.stringWidth("x 10");
        graphics.setFont(labelFont);
        graphics.drawString("x 10", xStartPosition, yStartPosition);
      }
    }
    ...
    if(yticks==null) {
      ...
      if(ylog||drawMinorYGrid) {
        ...
        ArrayList unlabeledgrid = gridInit(yStart, yTickSize, false, ygrid);
      ...
      if(yExponent!=0) {
        ...
        graphics.setFont(superscriptFont);
        graphics.drawString(Integer.toString(yExponent), labelFontMetrics.stringWidth("x 10")+2, vSpaceForTitle-halflabelheight);
        graphics.setFont(labelFont);
      }
    } else { // ticks have been explicitly specified
      for(Iterator nt = yticks.iterator(), nl = yticklabels.iterator();nl.hasNext();) {
        String label = (String) nl.next();
        double ypos = ((Double) (nt.next())).doubleValue();
    ...
    int charwidth = labelFontMetrics.stringWidth("8");
    if(xticks==null) { // auto-ticks
      ...
      if(xlog) {
        ...
        int count = 0;
        while(count++<=10) {
          xTickSize = roundUp((xtickMax-xtickMin)/(double) numberXTickMarks);
      ...
      xTickSize = (xlog) ? roundUp(0.8*(xtickMax-xtickMin)/(double) numberXTickMarks) :
                          roundUp((xtickMax-xtickMin)/(double) numberXTickMarks);
      numfracdigits = numFracDigits(xTickSize);
      ...
      if(xlog||drawMinorXGrid) {
        ...
        ArrayList unlabeledgrid = gridInit(xTmpStart, tmpStep, false, xgrid);
        if(unlabeledgrid.size()>0) {
      ...
      Iterator nt = xticks.iterator();
      Iterator nl = xticklabels.iterator();
      // Code contributed by Jun Wu (jwu@inin.com.au)
      double preLength = 0.0;
      while(nl.hasNext()) {
        String label = (String) nl.next();
        double xpos = ((Double) (nt.next())).doubleValue();
   ...
   * @return          Description of the Return Value
   */
  private ArrayList gridInit(double low, double step, boolean labeled, ArrayList oldgrid) {
    // How log axes work:
    ...
    // grid marks.
    ArrayList grid = new ArrayList(10);
    ...
    for(int i = 0;i<ngrid;i++) {
      ...
      if((oldgrid!=null)&&(oldgridi<oldgrid.size())) {
        ...
        // trying to add.
        while((oldgridi<oldgrid.size())&&((Double) oldgrid.get(oldgridi)).doubleValue()<logval) {
          oldgridi++;
        }
        if(oldgridi<oldgrid.size()) {
          // Using == on doubles is bad if the numbers are close,
          // but not exactly equal.
          if(Math.abs(((Double) oldgrid.get(oldgridi)).doubleValue()-logval)>0.00001) {
    ...
    for(gridCurJuke = -1;(gridCurJuke+1)<grid.size()&&(x>=((Double) grid.get(gridCurJuke+1)).doubleValue());gridCurJuke++) {}
    return grid;
  }
   ...
   * @return       Description of the Return Value
   */
  private double gridRoundUp(ArrayList grid, double pos) {
    double x = pos-Math.floor(pos);
    int i;
    for(i = 0;(i<grid.size())&&(x>=((Double) grid.get(i)).doubleValue());i++) {}
    if(i>=grid.size()) {
      return pos;
    } else {
      return Math.floor(pos)+((Double) grid.get(i)).doubleValue();
    }
  }
   ...
   * @return          Description of the Return Value
   */
  private double gridStep(ArrayList grid, double pos, double step, boolean logflag) {
    // W. Christian
    if(step==0) {
    ...
    if(logflag) {
      ...
      return gridBase+((Double) grid.get(gridCurJuke)).doubleValue();
    } else {
  ...
  private void setXRange(double min, double max) {
    ...
    if((xExponent>1)||(xExponent<-1)) {
      double xs = 1.0/Math.pow(10.0, (double) xExponent);
      xtickMin = xMin*xs;
      xtickMax = xMax*xs;
    } else {
      xtickMin = xMin;
  ...
  private void setYRange(double min, double max) {
    ...
    if((yExponent>1)||(yExponent<-1)) {
      double ys = 1.0/Math.pow(10.0, (double) yExponent);
      ytickMin = yMin*ys;
      ytickMax = yMax*ys;
    } else {
      ytickMin = yMin;
  ...
  public Dimension getInterior(DrawingPanel panel) {
    if(panel.getDimensionSetter()==null) {
      ...
      adjustGutters = false; // another object will set the gutters.
    }
    return null;
  }
}
