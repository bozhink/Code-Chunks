...
public class DrawingFrame extends OSPFrame implements ClipboardOwner {
         ...
         public void actionPerformed(ActionEvent e) {
            ...
            if(printerJob.printDialog()) {
               try {
                  ...
                  JOptionPane.showMessageDialog(DrawingFrame.this, DisplayRes.getString("DrawingFrame.PrintErrorMessage"), //$NON-NLS-1$
                                                DisplayRes.getString("DrawingFrame.Error"), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$
               }
            }
         }
      });
   ...
   protected JMenu loadToolsMenu(){
      ...
      videoItem.addActionListener(new ActionListener(){
        public void actionPerformed(ActionEvent e){
           VideoCaptureTool tool=drawingPanel.getVideoCaptureTool();
           if(tool==null){
              tool = VideoGrabber.getTool();
              drawingPanel.setVideoCaptureTool(tool);
              tool.setVisible(true);
              tool.clear();
           }else{
              tool.setVisible(true);
