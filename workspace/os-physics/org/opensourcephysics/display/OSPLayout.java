...
public class OSPLayout extends BorderLayout {
  ...
  public void addLayoutComponent(Component comp, Object constraints) {
    if(!list.contains(comp)) {
      list.add(comp);
      components = (Component[]) list.toArray(new Component[0]);
    }
    synchronized(comp.getTreeLock()) {
      if((constraints instanceof String)&&"TopLeftCorner".equals(constraints)) {
        list.remove(topLeftCorner);
        topLeftCorner = comp;
      } else if((constraints instanceof String)&&"TopRightCorner".equals(constraints)) {
        list.remove(topRightCorner);
        topRightCorner = comp;
      } else if((constraints instanceof String)&&"BottomLeftCorner".equals(constraints)) {
        list.remove(bottomLeftCorner);
        bottomLeftCorner = comp;
      } else if((constraints instanceof String)&&"BottomRightCorner".equals(constraints)) {
