...
public class InteractivePanel extends DrawingPanel implements InteractiveMouseHandler {
  ...
  void scaleX(ArrayList tempList){
     ...
     if ( autoscaleX && mouseAction==MOUSE_DRAGGED){
        if(xminPreferred>tempmin) xminPreferred = tempmin;
        if(xmaxPreferred<tempmax) xmaxPreferred = tempmax;
     }
  }
  ...
  void scaleY(ArrayList tempList){
     ...
     if (autoscaleY && mouseAction==MOUSE_DRAGGED){
        if(yminPreferred>tempmin) yminPreferred = tempmin;
        if(ymaxPreferred<tempmax) ymaxPreferred = tempmax;
     }
  }
