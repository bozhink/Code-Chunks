...
public class OSPLog extends JFrame {
  ...
  public static final Level[] levels = new Level[] {
    ...
    ConsoleLevel.ERR_CONSOLE, ConsoleLevel.OUT_CONSOLE, Level.CONFIG, Level.FINE, Level.FINER, Level.FINEST, Level.ALL
  };
  ...
  static {
    try { // system properties may not be readable in some environments
      ...
      tmpdir = System.getProperty("java.io.tmpdir", tmpdir); //$NON-NLS-1$
    } catch(SecurityException ex) {/** empty block */}
  }
  ...
  public static OSPLog getOSPLog() {
    if(OSPLOG==null) {
      ...
      if(!OSPFrame.appletMode && OSPFrame.applet==null) { // cannot redirect applet streams
        try {
  ...
  public static void showLog() {
    if(OSPFrame.appletMode || OSPFrame.applet!=null) {
      org.opensourcephysics.controls.MessageFrame.showLog(true);
    } else {
    ...
    if(result==JFileChooser.APPROVE_OPTION) {
      ...
      if(file.exists()) {
        int selected = JOptionPane.showConfirmDialog(this,
           ControlsRes.getString("OSPLog.ReplaceExisting_dialog_message")+file.getName()+ControlsRes.getString("OSPLog.question_mark"), ControlsRes.getString("OSPLog.ReplaceFile_dialog_title"), JOptionPane.YES_NO_CANCEL_OPTION); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
...
/*  //Uncomment this method to test the OSPLog.
  ...
  protected synchronized Handler getFileHandler() {
    ...
    try {
      ...
      tempFileName = tmpdir+pkgName+".log"; //$NON-NLS-1$
      fileHandler = new FileHandler(tempFileName);
      fileHandler.setFormatter(new XMLFormatter());
      fileHandler.setLevel(Level.ALL);
      logger.addHandler(fileHandler);
      logger.log(Level.FINE, "Logging to file enabled. File = "+tempFileName); //$NON-NLS-1$
  ...
  protected void createMenus() {
    ...
    openAction.setEnabled(!OSPFrame.appletMode &&  OSPFrame.applet==null);
    ...
    saveAsAction.setEnabled(!OSPFrame.appletMode && OSPFrame.applet==null);
    ...
    logToFileItem.setSelected(false);
    logToFileItem.setEnabled(!OSPFrame.appletMode &&  OSPFrame.applet==null);
...
class OSPLogHandler extends Handler {
  ...
  public void flush() {/** empty block */}

  public void close() {/** empty block */}
}
