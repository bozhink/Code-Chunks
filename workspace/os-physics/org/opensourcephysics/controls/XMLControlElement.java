...
public class XMLControlElement implements XMLControl {
   ...
   */
  public XMLControlElement() {/** empty block */}
   ...
   * @param lock boolean
   */
  public void setLockValues(boolean lock) {/** empty block */}
    ...
    if(prop!=null&&prop.getPropertyType().equals("string")) { //$NON-NLS-1$
      ...
      if(content.indexOf(XML.CDATA_PRE)!=-1) {
      	content = content.substring(
      					content.indexOf(XML.CDATA_PRE)+XML.CDATA_PRE.length(),
  ...
  public void setDecryptPolicy(int policy) {
  	if (policy == NEVER_DECRYPT) decryptPolicy = NEVER_DECRYPT;
  	else if (policy == PASSWORD_DECRYPT) decryptPolicy = PASSWORD_DECRYPT;
  	else decryptPolicy = ALWAYS_DECRYPT;
  }
  ...
  public String readForClass(String name, Class type) {
    ...
    if (!isInputForClass(type)) return null;
    return read(name);
  }
  ...
  public boolean readXMLForClass(String xml, Class type) {
    input = new BufferedReader(new StringReader(xml));
    if (!isInputForClass(type)) return false;
    readXML(xml);
    return !readFailed;
  }
  ...
  public Object loadObject(Object obj, boolean autoImport, boolean importAll) {
      ...
      if(obj!=null) {
        if(!autoImport) {
          int result = JOptionPane.showConfirmDialog(null, 
          				ControlsRes.getString("XMLControlElement.Dialog.UnknownClass.Message")+" \""+className+"\""+XML.NEW_LINE //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    if(obj!=null&&!type.isInstance(obj)) {
      if(!autoImport) {
        int result = JOptionPane.showConfirmDialog(null, 
        				ControlsRes.getString("XMLControlElement.Dialog.MismatchedClass.Message")+" \""+type.getName()+"\""+XML.NEW_LINE //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        				+ControlsRes.getString("XMLControlElement.Dialog.MismatchedClass.Query")+" \""+obj.getClass().getName()+"\"",  //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        				ControlsRes.getString("XMLControlElement.Dialog.MismatchedClass.Title"),  //$NON-NLS-1$
        				JOptionPane.YES_NO_OPTION, 
        				JOptionPane.WARNING_MESSAGE);
   ...
   * Method required by the Control interface.
   */
  public void clearMessages() {/** empty block */}
   ...
   * @param s the string
   */
  public void calculationDone(String s) {/** empty block */}
   ...
   */
  public void setValue(String stringValue) {/** empty block */}
  ...
  private boolean importInto(Object obj, boolean importAll) {
    ...
    ListChooser chooser = new ListChooser(ControlsRes.getString("XMLControlElement.Chooser.ImportObjects.Title"), ControlsRes.getString("XMLControlElement.Chooser.ImportObjects.Label")); //$NON-NLS-1$ //$NON-NLS-2$
    	...
    	// return if decrypt policy is NEVER or unverified PASSWORD
		  switch (decryptPolicy) {
		  	...
		  	case PASSWORD_DECRYPT:
		    		...
		    		if (!Password.verify(password, null)) return;
					}
		  }
