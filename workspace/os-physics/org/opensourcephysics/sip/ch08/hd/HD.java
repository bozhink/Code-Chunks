...
// step
  public void step() {
    ...
    move();  // move particles for time = minimum collision time
    t += timeToCollision;
    contact();  // change velocities of two colliding particles
    // set all collision times to bigTime for those particles set to collide with
    ...
    // and if set new collision times as appropriate
    newCollisionTimes();
    numberOfCollisions++;
  }
  ...
  public void move() {
    for(int k = 0;k<N;k++) {
      ...
      y[k] = centerCell(y[k]+vy[k]*timeToCollision, Ly);  // not in periodic image
    }
  }
