...
//_____________________________ private methods _______________________
  ...
  */
  ...
  private void setOutputToField() {
    ...
    if (h % 2 != 0) h = h - 1; // make sure we have an even number of rows
      // copy pixels in every other row
    for (int i = 0; i < h/2 - 1; i++) {
      for (int j = 0; j < w; j++) {
        if (isOdd) pixels[w*2*i + j] = pixels[w*(2*i+1) + j];
        else pixels[w*(2*i+1) + j] = pixels[w*2*i + j];
      }
    }
  ...
  private class Inspector extends JDialog {
     ...
     */
    void updateDisplay() {
      if (isOdd) group.setSelected(odd.getModel(), true);
      else group.setSelected(even.getModel(), true);
    }
  ...
  static class Loader implements XML.ObjectLoader {
    ...
    public void saveObject(XMLControl control, Object obj) {
      ...
      if (filter.isOdd()) control.setValue("field", "odd"); //$NON-NLS-1$ //$NON-NLS-2$
      else control.setValue("field", "even"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public Object loadObject(XMLControl control, Object obj) {
      ...
      if (control.getPropertyNames().contains("field")) { //$NON-NLS-1$
        if (control.getString("field").equals("odd")) filter.setOdd(true); //$NON-NLS-1$ //$NON-NLS-2$
        else filter.setOdd(false);
      }
