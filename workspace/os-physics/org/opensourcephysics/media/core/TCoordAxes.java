...
public class TCoordAxes extends TShape {
  ...
  public void draw(DrawingPanel panel, Graphics g) {
    if (panel == vidPanel) super.draw(panel, g);
  }
  ...
  public Interactive findInteractive(
    ...
    if (panel != vidPanel) return null;
    if (!isEnabled() || !isVisible()) return null;
    setHitRectCenter(xpix, ypix);
    if (originEnabled && originShape.intersects(hitRect)) return origin;
    if (xaxisEnabled && xaxis.intersects(hitRect)) return this;
    return null;
  }
