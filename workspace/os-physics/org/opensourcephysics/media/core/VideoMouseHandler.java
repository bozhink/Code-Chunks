...
public class VideoMouseHandler implements InteractiveMouseHandler {
  ...
  public void handleMouseAction(InteractivePanel panel,
                                MouseEvent e) {
    if (!(panel instanceof VideoPanel)) return;
    ...
    switch(vidPanel.getMouseAction()) {
      ...
      case InteractivePanel.MOUSE_MOVED:
        ...
        else vidPanel.setMouseCursor(Cursor.getDefaultCursor());
        break;
    }
    if (p == null) vidPanel.hideMouseBox();
    else p.showCoordinates(vidPanel);
  }
}
