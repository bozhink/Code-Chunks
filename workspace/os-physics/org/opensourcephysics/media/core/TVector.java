...
public class TVector extends TShape {
  ...
  public Interactive findInteractive(
    ...
    if (!(panel instanceof VideoPanel)) return null;
    if (!isEnabled() || !isVisible()) return null;
    setHitRectCenter(xpix, ypix);
    if (shaft != null && shaft.intersects(hitRect)) return this;
    if (tipEnabled && head != null && head.intersects(hitRect)) return tip;
    return null;
  }
