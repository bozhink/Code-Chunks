...
public abstract class ControlSwingElement extends ControlElement {
  ...
  static public final int NAME = 0;       // The name of the element
  ...
  static private ArrayList myInfoList = null; // The list of registered properties
    ...
    case PARENT : {
      ...
      if(parent==null) {
        if(!(this instanceof ControlWindow)) {
          System.err.println(getClass().getName()+" : Error! Parent <"+_value+"> not found for "+toString());
        }
      } else {
        if(parent instanceof ControlContainer) {
          ((ControlContainer) parent).add(this);
        } else {
          System.err.println(getClass().getName()+" : Error! Parent <"+_value+"> is not a ControlContainer");
        }
      }
    }
    break;
  ...
  protected javax.swing.ImageIcon getIcon(String _iconFile) {
    ...
    if(getProperty("_ejs_codebase")!=null) {
      icon = Utils.icon(getProperty("_ejs_codebase"), _iconFile);
