...
public abstract class ControlElement {
  ...
  protected Hashtable myPropertiesTable = new Hashtable(); // A place to hold any property
  protected Object myObject = null;            // The wrapped object
  private boolean myActiveState = true;        // Whether I am active or not
  private Vector myActionsList = new Vector(); // My list of actions
  ...
  public static final String METHOD_TRIGGER = "_expr_"; // AMAVP
  ...
  public Value parseConstant(String _propertyType, String _value) {
    ...
    if(_propertyType.indexOf("File")>=0) {
      ...
      if(getProperty("_ejs_codebase")!=null) {
        codebase = getProperty("_ejs_codebase");
    ...
    if(_propertyType.indexOf("Margins")>=0||_propertyType.indexOf("Rectangle")>=0) {
  ...
  public ControlElement setProperty(String _property, String _value) {
    ...
    if(!propertyIsTypeOf(_property, "NotTrimmed")) {
      _value = _value.trim();
    }
    ...
    if(_value.startsWith("%")&&_value.endsWith("%")&&_value.length()>2) {
      ...
      ; // Do nothing for parsed expressions
    } else if(_value.startsWith("#")&&_value.endsWith("#")&&_value.length()>2) {
      ;   // Do nothing for variables such as f()
    } else {
      if(_value.startsWith("\"")||_value.startsWith("'")) {
        ; // It IS a constant String, don't try anything else
      } else {
        // First look for a CONSTANT property that can not be associated to GroupVariables
        if(propertyIsTypeOf(_property, "CONSTANT")) {
          constantValue = new StringValue(_value);
        }
        // Check for String properties
        if(constantValue==null) {
          if(propertyType(_property).equals("String")&&!propertyIsTypeOf(_property, "VARIABLE_EXPECTED")) { // See TextField f.i.
            constantValue = new StringValue(_value);
          }
        }
      }
    ...
    if(constantValue!=null) { // Just set the value for this property
      ...
      if (constantValue instanceof StringValue && propertyIsTypeOf(_property, "TRANSLATABLE")) { // Apply TranslatorTool
        Object target = null;
        if (myGroup!=null) target = myGroup.getTarget("_default_");
        String translated = org.opensourcephysics.tools.TranslatorTool.getProperty(target.getClass(),constantValue.getString());
        if (!constantValue.getString().equals(translated)) constantValue = new StringValue(translated);
      }
      ...
      if(myGroup!=null) {
        ...
        if(isNormalVariable) {      // Connect a variable property with a normal variable name
          ...
          if(newValue==null) {
            ...
            // else
            if(propertyIsTypeOf(_property, "double")) {
              newValue = new DoubleValue(0.0);
            } else if(propertyIsTypeOf(_property, "boolean")) {
              newValue = new BooleanValue(false);
            } else if(propertyIsTypeOf(_property, "int")) {
              newValue = new IntegerValue(0);
            } else if(propertyIsTypeOf(_property, "String")) {
          ...
          if(propertyIsTypeOf(_property, "double")) {
            ...
            returnType = "String";
          } else if(propertyIsTypeOf(_property, "Action")) {
            returnType = "Action";
          } else {
          ...
          if(getProperty("_ejs_")!=null) { // Do nothing in Ejs
  ...
  final public ControlElement setProperties(String _propertyList) {
    ...
    while(tkn.hasMoreTokens()) {
      ...
      if(index<0) {
        System.err.println(getClass().getName()+" : Error! Token <"+token+"> invalid for "+toString());
      } else {
  ...
  private ControlElement setProperties(Hashtable _propertyTable) {
    ...
    for(Enumeration e = _propertyTable.keys();e.hasMoreElements();) {
      String key = (String) e.nextElement();
      // Some need to be processed before the others
      if(propertyIsTypeOf(key, "PREVIOUS")) {
    ...
    for(Enumeration e = _propertyTable.keys();e.hasMoreElements();) {
      String key = (String) e.nextElement();
      setProperty(key, (String) _propertyTable.get(key));
    }
    // Finally proccess those which need to be the last ones
    for(Enumeration e = postTable.keys();e.hasMoreElements();) {
      String key = (String) e.nextElement();
      setProperty(key, (String) postTable.get(key));
    }
    return this;
  }
  ...
  final public String getProperty(String _property) {
    return(String) myPropertiesTable.get(_property);
  }
  ...
  public String toString() {
    String name = (String) myPropertiesTable.get("name");
    if(name!=null) {
      return name;
    }
  ...
  final public ControlElement addAction(int _type, String _method) {
    ...
    if(myGroup!=null) {
      ...
      if(_type==ACTION&&getProperty("_ejs_SecondAction_")!=null&&myGroup.getTarget("_default_")!=null) {
        secondAction = new MethodWithOneParameter(_type, myGroup.getTarget("_default_"), getProperty("_ejs_SecondAction_"), null, null, this); // null = void , null= no 2nd action
      }
    }
    if(target==null) {
      System.err.println(getClass().getName()+" : Error! Target <"+parts[0]+"> not assigned");
    ...
    if(parts[2]==null) {
      return addAction(_type, target, parts[1]+"()", secondAction);
    } else {
  ...
  final public void removeAction(int _type, Object _target, String _method) {
    ...
    for(Enumeration e = myActionsList.elements();e.hasMoreElements();) {
