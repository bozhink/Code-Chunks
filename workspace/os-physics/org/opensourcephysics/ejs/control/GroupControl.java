...
public class GroupControl {
  ...
  public GroupControl() {
    ...
    int index = debugPrefix.lastIndexOf(".");
    if(index>=0) {
    ...
    methodTriggerVariable = (GroupVariable) variableTable.get(ControlElement.METHOD_TRIGGER); // AMAVP
  }
  ...
  public void setValue(String _name, Value _value) {
    ...
    GroupVariable variable = (GroupVariable) variableTable.get(_name);
    if(variable==null) {
  ...
  public void addListener(String _name, String _method, Object _anObject) {
    if((debugLevel&DEBUG_SET_AND_GET)>0) {
      System.out.print(debugPrefix+": Adding listener for variable <"+_name+"> to <"+_method+"> ...");
    }
    if(_name==null) {
      return;
    }
    ...
    if(target==null) {
      ...
      System.out.print(debugPrefix+": Target <"+parts[0]+"> found. Method is <"+_method+"> ...");
    }
    GroupVariable variable = (GroupVariable) variableTable.get(_name);
    if(variable==null) {
  ...
  final public ControlElement addNamed(String _type, String _name) {
    ...
    if(_type.endsWith("ControlFrame")||_type.endsWith("ControlDrawingFrame")) {
      ...
      return addObject(null, "org.opensourcephysics.ejs.control.swing.ControlPanel", propertyList);
    }
    return addObject(null, _type, propertyList);
  }
  ...
  public ControlElement addObject(Object _object, String _type, String _propertyList) {
    ...
    if(_type.indexOf(".")<0) {
      for(Enumeration e = prefixList.elements();e.hasMoreElements()&&element==null;) {
        element = instantiateClass(_object, ((String) e.nextElement())+_type, false); // Silently
      }
    }
    if(element==null) {
    ...
    if(element instanceof ControlWindow) { // Make windows visible by default
      if(element.getProperty("visible")==null) {
  ...
  public ControlElement getElement(String _name) {
    ...
    ControlElement element = (ControlElement) elementTable.get(_name);
    if(element==null) {
  ...
  public void update() {
    ...
    for(Enumeration e = updateList.elements();e.hasMoreElements();) {
  ...
  private void destroy(ControlElement _element, boolean _informMyParent) {
    ...
    if(_element instanceof ControlContainer) {
      for(Enumeration e = ((ControlContainer) _element).getChildren().elements();e.hasMoreElements();) {
  ...
  static class GroupControlLoader implements XML.ObjectLoader {
    ...
    public void saveObject(XMLControl control, Object obj) {
      ...
      while(it.hasNext()) {
        ...
        if(!name.startsWith("_")) { // don't save ejs internal variables
