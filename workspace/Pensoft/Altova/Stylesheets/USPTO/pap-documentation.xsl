<?xml-stylesheet type="text/xsl" href="pap-documentation.xsl"?>
<xsl:stylesheet 
...
<xsl:import href="pap.xsl" doc:description="The &quot;driver&quot; file for the XSL tranformation resulting in an HTML document
         from a &lt;patent-application-publication&gt; XML document instance.    
         The pap.xsl file calls its child templates through &lt;xsl:include&gt;"/>

<xsl:output method="xml" indent="yes" media-type="text/html"/>

    
    <doc:summary>
        <doc:title>Patent Application Publication Documentation</doc:title>
        <doc:filename>pap-documentation.xsl</doc:filename>
        ...
        for the Patent Application Publication doctype.
        </doc:description>
    </doc:summary>
   ...
   Kurt Cagle, customized for USPTO by John Dunning, 2001.  For use in commercial products, please contact the author for permissions.">
    ...
    <xsl:template match="/">
        <html>
        <head><title>Patent Application Stylesheet Documentation</title></head>
            <body>
            <basefont size="2">
            <style>
            td {font-size: 10pt;}
    ...
    <xsl:template match="xsl:template[@match and @doc:public='yes']">
        ...
        <ul>
        <xsl:for-each select="xsl:param[not(@doc:private='yes')]">
            <li><span style="background-color:#ff00ff;"><b><xsl:value-of select="@name"/>:</b></span>&#160;&#160;&#160;<xsl:value-of select="@doc:description"/>
            <xsl:choose>
                <xsl:when test="@select">
                    <i>[default=<xsl:value-of select="@select"/>]</i>
                </xsl:when>
