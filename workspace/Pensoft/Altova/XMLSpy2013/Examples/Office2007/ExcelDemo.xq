...
declare variable $rels-doc :=  doc(concat($xlsx-uri, '|zip\_rels\.rels'));

declare variable $documentFileUri :=  concat($xlsx-uri, '|zip\', $rels-doc/r:Relationships/r:Relationship[@Type=$officeDocmentURI]/@Target);
declare variable $rels-documentFileUri :=  concat ($xlsx-uri, '|zip\xl\_rels\workbook.xml.rels');
declare variable $sharedStrings-FileUri :=  concat($xlsx-uri, '|zip\xl\',local:getRelationship( $rels-documentFileUri ,$sharedStringsURI)/@Target);
...
{ doc(concat ( $xlsx-uri, '|zip\xl\worksheets\sheet', doc($documentFileUri)/workbook/sheets/sheet[@name=$sheetName]/@sheetId,'.xml'))};
