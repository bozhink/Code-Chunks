...
ggplot(data.frame(meuse), aes(dist, zinc)) + 
  geom_point(size = 2, color = 'blue', alpha = 0.3)+
  ggtitle('Concentration of zinc vs. distance') +
  ylab('Zinc concentration in ppm') +
  xlab('Distance from river')
...
plot(copper.vgm,
     copper.vgm.fit,
     main = "Semivariance of copper concentration vs. distance")

copper.vgm.dir = variogram(logCopper ~ sqrtDist, meuse, alpha = c(0, 45, 90, 135))
plot(copper.vgm.dir,
     copper.vgm.fit,
     main = 'Variograms of copper concentration at different angles')

copper.vgm.map = variogram(logCopper ~ sqrtDist, meuse,
                           cutoff = 1500, width = 100, map = TRUE)
plot(copper.vgm.map,
     threshold = 5,
     main = "Map of semivariance of copper concentration")
...
plot(zinc.vgm,
     zinc.vgm.fit,
     main = "Semivariance of zinc concentration vs. distance")

zinc.vgm.dir = variogram(logZinc ~ sqrtDist, meuse, alpha = c(0, 45, 90, 135))
plot(zinc.vgm.dir,
     zinc.vgm.fit,
     main = 'Variograms of zinc concentration at different angles')

zinc.vgm.map = variogram(logZinc ~ sqrtDist, meuse,
                           cutoff = 1500, width = 100, map = TRUE)
plot(zinc.vgm.map,
     threshold = 5,
     main = "Map of semivariance of zinc concentration")
...
par(mar = c(0.7,0.7,0.7,0.7))
spplot(copper.idw["var1.pred"],
       main = "Inverse distance weighted interpolation for copper concentration")

copper.kriged = krige(logCopper ~ 1, meuse, meuse.grid, model = copper.vgm.fit)
spplot(copper.kriged["var1.pred"],
       main ="Plot of kriging model of copper concentration")
...
spplot(zinc.idw["var1.pred"],
       main = "Inverse distance weighted interpolation for zinc concentration")

zinc.kriged = krige(logZinc ~ 1, meuse, meuse.grid, model = zinc.vgm.fit)
spplot(zinc.kriged["var1.pred"],
       main ="Plot of kriging model of zinc concentration")
